/**
 * NeighborhoodChart — نمودار خطی قیمت هر مترمربع (تومان + دلار) — SVG داخلی
 * فقط از Snapshotهای واقعی ثبت‌شده استفاده می‌کند.
 */
...
export function NeighborhoodChart({ points }: { points: ChartPoint[] }) {
  if (points.length === 0) {
    return (
  ...
  return (
    <div className="space-y-3">
      <div>
        <div className="mb-1 flex items-center justify-between text-[9px] font-bold">
          <span className="flex items-center gap-1 text-muted">
            <span className="h-2 w-2 rounded-full bg-accent" /> قیمت هر مترمربع (تومان)
          </span>
      ...
      <div>
        <div className="mb-1 flex items-center justify-between text-[9px] font-bold">
          <span className="flex items-center gap-1 text-muted">
            <span className="h-2 w-2 rounded-full bg-indigo-400" /> قیمت هر مترمربع (دلار — ثبت‌شده)
          </span>
      ...
      <div className="flex justify-between text-[8px] font-bold text-muted">
        {points.map((p, i) => (
          <span key={i} className="truncate px-0.5">{p.label}</span>
        ))}
      </div>
    </div>
  );
